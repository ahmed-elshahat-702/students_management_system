...
const ScheduleTable = ({ title, tableData, grade, colorGradient }) => {
  return (
    <div className="bg-white shadow-lg rounded-lg pt-6">
      <h2 className="text-3xl font-bold text-gray-800 mx-6 mb-6">{title}</h2>
      <div className="overflow-x-auto">
        {tableData && Object.keys(tableData).length !== 0 ? (
          <table className="min-w-full table-auto text-left border-collapse">
            ...
            <tbody>
              {Object.keys(tableData).map((day, dayIndex) => {
                const daySessions = tableData[day];
                return (
          ...
          <p className="text-center text-red-500 text-xl py-6">
            No {title.toLowerCase()} for {grade}
          </p>
        )}
      </div>
    </div>
